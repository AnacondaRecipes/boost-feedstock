...
# Check if file exists in PREFIX (from installed libboost-headers package)
if [[ -f $PREFIX/lib/cmake/boost_headers-$PKG_VERSION/boost_headers-config.cmake ]]; then
    mv $PREFIX/lib/cmake/boost_headers-$PKG_VERSION/boost_headers-config.cmake $SRC_DIR/cf_cmake/
elif [[ -f $SRC_DIR/temp_prefix/lib/cmake/boost_headers-$PKG_VERSION/boost_headers-config.cmake ]]; then
    # If not in PREFIX, try temp_prefix from main build
    cp $SRC_DIR/temp_prefix/lib/cmake/boost_headers-$PKG_VERSION/boost_headers-config.cmake $SRC_DIR/cf_cmake/
fi
...
if [[ -f $SRC_DIR/cf_cmake/boost_headers-config.cmake ]] && [[ -d $PREFIX/lib/cmake/boost_headers-$PKG_VERSION ]]; then
...
if [[ -d $PREFIX/lib/cmake/boost_python-$PKG_VERSION ]]; then
    mv $PREFIX/lib/cmake/boost_python-$PKG_VERSION $SRC_DIR/cf_${PY_VER}_${python_impl}_cmake/
fi
if [[ -d $PREFIX/lib/cmake/boost_numpy-$PKG_VERSION ]]; then
